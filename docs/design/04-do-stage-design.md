...
my-cc-lite 原生接管默认一次 `/do` 连续推进当前 active task 中所有可执行的 task。外部高阶接管由外部流程决定如何推进完整 `tasks[]`。无论哪种接管方式，每次状态写入仍只更新一个 task；用户再次调用 `/do` 时，先从已有 `task.json` 恢复状态并选择当前 task，只有本轮明确继续执行时才处理剩余可执行 task。
...
选择 my-cc-lite 原生接管，或已有 `task.json` 且用户明确继续执行时，`/do` 使用内置 task loop 推进 `tasks[]`。
