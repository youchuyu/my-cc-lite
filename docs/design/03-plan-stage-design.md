...
- `/do` 首次执行时再根据最新 `plan.md` 创建或更新 `task.json`。
...
- `Plan`：按人类可读方式列出主要工作项。每个工作项应包含 `Goal`、`Do` 和 `Check`，复杂任务下可以补充局部 `Scope`。
...
`plan.md` 不写 `status`，不写 JSON，不要求 `/do` 一比一同步，也不写文件级 TODO。后续 `/do` 会读取最新 `plan.md`，再根据实际代码上下文拆分、合并或调整执行阶段需要的 `tasks[]`、`steps[]` 和 `checks[]`。
...
用户可以直接编辑 `plan.md`。`/do` 执行时读取最新 `plan.md`，再创建或更新 `task.json`。
